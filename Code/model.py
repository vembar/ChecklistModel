...
class GRU(nn.Module):
    ...
    def Initialize_params(self):
        ...
        self.u_z  = orthogonal_init((hidden_dim,hidden_dim))
        self.b_z = uniform_init((self.hidden_dim))

        self.w_r = uniform_init((self.input_dim, self.hidden_dim))
        self.u_r  = orthogonal_init((hidden_dim,hidden_dim))
        self.b_r = uniform_init((self.hidden_dim))

        self.w_c = uniform_init((self.input_dim, self.hidden_dim))
        self.u_c  = orthogonal_init((hidden_dim,hidden_dim))
        self.b_c = uniform_init((self.hidden_dim))

        self.z = uniform_init((hidden_dim,hidden_dim))
        self.y = uniform_init((hidden_dim,hidden_dim))
        self.w_s = uniform_init((hidden_dim,hidden_dim))
        self.u_s = uniform_init((hidden_dim,hidden_dim))
        self.w_q = uniform_init((hidden_dim,hidden_dim))
        self.u_q = uniform_init((hidden_dim,hidden_dim))
        self.u_g = uniform_init((hidden_dim,hidden_dim))

    def __init__(self,input_dim,hidden_dim,name='GRU'):
        """
        ...
        #self.h_0 = nn.Parameter(torch.Tensor(hidden_dim))

        # changes in GRU unit to incorporate checklists
        self.z = nn.Parameter(torch.Tensor(hidden_dim,hidden_dim))
        self.y = nn.Parameter(torch.Tensor(hidden_dim,hidden_dim))
        self.w_s = nn.Parameter(torch.Tensor(hidden_dim,hidden_dim))
        self.w_q = nn.Parameter(torch.Tensor(hidden_dim,hidden_dim))
        self.u_s = nn.Parameter(torch.Tensor(hidden_dim,hidden_dim))
        self.u_q = nn.Parameter(torch.Tensor(hidden_dim,hidden_dim))
        self.u_g = nn.Parameter(torch.Tensor(hidden_dim,hidden_dim))

    def forward(self, input, g, et_new):
        ...
        def recurrence(x_t,h_t,g,et_new):
            ...
            h_t1 = (1 - z_t) * h_t1 + z_t * c_t
            return h_t1, c_t

        hidden = torch.randn(hidden_dim,1)
        hidden = F.linear(h_0,self.u_g)

        output = []
        steps = range(input.size(0))
        for i in steps:
            hidden = recurrence(input,g,et_new)
